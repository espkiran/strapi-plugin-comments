...
const AbuseReportsPopUp = ({
  ...
  const resolveReportReason = (reason) =>
    formatMessage({
      id: `${pluginId}.popup.reports.item.reason.${reason.toLowerCase()}`,
    });
  ...
  const headers = [
    ...
    {
      name: formatMessage({
        id: `${pluginId}.popup.reports.item.header.reason`,
      }),
      value: "reason",
    },
    {
      name: formatMessage({
        id: `${pluginId}.popup.reports.item.header.message`,
      }),
      value: "content",
    },
    { value: "actions" },
  ...
  return (
    <CheckPermissions permissions={pluginPermissions.moderateReports}>
      <Modal isOpen={isOpen} onToggle={onClose}>
                  ...
                  <Table
                    customRow={(props) => (
                      <CustomRow
                        ...
                        onClick={(e) =>
                          onAbuseReportResolveClick(e, props.row.id)
                        }
                      />
                    )}
                    headers={headers}
        ...
        {!isEmpty(reports) && !blockedThread && !blocked && (
          <ModalFooter>
            <section>
              <label>
                <FormattedMessage
                  id={`${pluginId}.popup.reports.footer.actions`}
                />
              </label>
              {!blockedThread && (
                <ButtonModal
