...
const DataManagerProvider = ({ children }) => {
    ...
    try {
      if (activeId) {
        ...
        const activeItem = await request(
          `/${pluginId}/moderation/single/${activeId}`,
          {
            method: "GET",
            signal,
          }
  ...
  getDataRef.current = async (searchParams = {}, recentlyViewed) => {
    try {
      ...
      const result = await request(
        `/${pluginId}/moderation/all?${generatedSearch}`,
        {
          method: "GET",
          signal,
        }
  ...
  useEffect(() => {
    getDataRef.current(
      getSearchParams(),
      moment(storeData.get(RECENTLY_VIEVEW_KEY) || undefined)
    );
  ...
  const blockComment = async (id) => {
    try {
      ...
      strapi.notification.success(
        `${pluginId}.notification.comment.visibility`
      );
    } catch (err) {
  ...
  const resolveAbuseReport = async (id, commentId) => {
    try {
      ...
      await request(
        `/${pluginId}/moderation/abuse-reports/${commentId}/resolve/${id}`,
        {
          method: "PATCH",
          signal,
        }
      ...
      strapi.notification.success(
        `${pluginId}.notification.abuse-report.resolution`
      );
    } catch (err) {
